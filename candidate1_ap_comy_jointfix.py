"""
Candidate 1 (stay-penalty variant + JOINT-LIMIT-AWARE PENALTY):
energy + tracking, NO CoM-based safety term, + com_y stabilization
+ active stay-near-start penalty + joint-limit proximity penalty.

WHY THIS PATCH WAS ADDED
---------------------------
Diagnostic evaluation (see thesis Section 4.3 diagnostics) found Candidate
1 exhibits a 100%-consistent failure for target=-0.08 (episodes ending
within ~150 steps), driven by hip_flexion approaching its own asymmetric
joint range (-30 deg to +120 deg). Candidates 2 and 3 (CoM-based safety
terms) do NOT exhibit this failure, which is itself the documented finding
that H3 (joint-asymmetry hypothesis) was not supported in its original
form. This patch is applied to Candidate 1 for THREE reasons, stated
explicitly so the intent is not confused with "fixing a bug":
  1. Completeness -- Candidate 1 is the only candidate with NO safety
     mechanism of any kind; adding a joint-limit-aware term gives it SOME
     safety mechanism, making the three-way comparison symmetric.
  2. Direct test of the joint-limit-aware approach specifically on the
     ONE candidate that actually exhibits the joint-limit failure --
     Candidates 2/3 never needed this fix, so applying it there (as was
     done experimentally, see Candidate 2's jointfix variant) could not
     test whether joint-limit-awareness actually solves the ORIGINAL
     failure. This file is the first genuine test of that.
  3. Consistency -- identical penalty formula, weight, and edge-zone
     parameters as used in Candidate 2's jointfix variant, for a fair
     comparison if this candidate is later evaluated alongside it.

WHAT CHANGES FROM THE ORIGINAL CANDIDATE 1
-----------------------------------------------
ONLY the addition of _joint_limit_penalty() and its inclusion in the
reward combination line. Tracking, stay-penalty, com_y stabilization,
energy, and all other logic are UNCHANGED from the original Candidate 1.
"""
import os
...
# Identical to Candidate 2's jointfix variant, for direct comparability.
JOINT_EDGE_ZONE = 0.15
JOINT_LIMIT_WEIGHT = 1.0


class Candidate1Env(AnkleHipEnv):
    """AP target-reaching, NO CoM safety term, + com_y stabilization
    ...
    def __init__(self, target_x_low=TARGET_X_LOW, target_x_high=TARGET_X_HIGH,
        ...
        # Precompute each joint's own FULL range (not the fail_low/high
        # margin) for the limit-proximity penalty, so the "danger zone"
        # starts a bit before the hard failure boundary.
    ...
    def _joint_limit_penalty(self, q):
        """Smooth, per-joint penalty that grows as any joint approaches
        EITHER of its own limits, normalized per-joint so the asymmetric
        hip_flexion range is handled correctly. Identical formula to
        Candidate 2's jointfix variant."""
    ...
    def step(self, action):
        ...
        # No CoM-based safety term -- Candidate 1 remains the no-CoM-safety
        # baseline. The ONLY new term is the joint-limit penalty below.
        safety = 0.0
...
if __name__ == "__main__":
    log_dir = "./training_logs_candidate1_ap_comy1_staypenalty_jointfix/"
    ...
    model.save("ppo_candidate1_ap_comy1_staypenalty_jointfix")
    env.save("vecnormalize_candidate1_ap_comy1_staypenalty_jointfix.pkl")
    env.close()
