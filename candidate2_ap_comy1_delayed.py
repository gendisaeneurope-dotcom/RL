"""
Candidate 2: energy + tracking (from Candidate F) + XCoM safety term + com_y stabilization.
...
# Safety weight: chosen after sweeping 0.1/0.15/0.2/0.25/0.3, 0.25 gave the best
# combination of zero grid failures and lowest perturbed reward drop.
SAFETY_WEIGHT = 0.25

# NEW: com_y (mediolateral) stabilization weight. Normalized the same way as
...
# sweep. Same value as SAFETY_WEIGHT, reusing the scale that's already
# confirmed to work well in this reward.
COM_Y_WEIGHT = 1.0
...
class Candidate2Env(AnkleHipEnv):
    ...
    def __init__(self, target_x_low=TARGET_X_LOW, target_x_high=TARGET_X_HIGH, fixed_target=None, omega=OMEGA,
                 ...
                 disturb_prob=0.1, force_range=(0, 30), **kwargs): #adding value to distturb_prob and determine posterior lean direction (force_range) = positive direction (plantarflexion)
    ...
    def step(self, action):
        ...
        # tracking: unchanged from Candidate 1, this is what stayed fixed
        ...
        # NEW: com_y stabilization. Nothing else in this reward touches the
        # mediolateral axis, so without this term the policy is free to
        # drift arbitrarily -- confirmed empirically across candidates
        # (mean final |com_y| ranged 0.04-0.24 across candidates/weights with
        # no penalty). Normalized by base_half_length, squared, same scale
        # convention as the safety term above, so one weight works across
        # all candidates without a separate sweep.
...
if __name__ == "__main__":
    log_dir = "./training_logs_candidate2_ap_comy1_delayed/"
    ...
    model.save("ppo_candidate2_ap_comy1_delayed")
    env.save("vecnormalize_candidate2_ap_comy1_delayed.pkl")
    env.close()
