"""
Candidate 1_E: target-reaching, no safety term. (tracking ÷ target_range stays, effort → action-based); add a joint-weight vector inside the effort calculation
...
JOINT_WEIGHTS = np.array([1.0, 3.0, 1.0, 3.0])  # extra weight on untracked sagittal joints (aflex, hflex)
...
class Candidate1Env(AnkleHipEnv):
    ...
    def step(self, action):
        ...
        if failed:
            ...
            ef_u = float(np.sum(JOINT_WEIGHTS * np.square(action)) / np.sum(JOINT_WEIGHTS))
...
if __name__ == "__main__":
    ...
    model.save("ppo_candidate1_E")
    env.save("vecnormalize_candidate1_E.pkl")
    env.close()
