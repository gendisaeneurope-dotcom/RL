"""
Candidate 1_B: target-reaching, no safety term. (tracking ÷ eps_pos, effort stays torque-based)
...
EPS_VEL = 0.01             # m/s, still an unsourced placeholder
...
class Candidate1Env(AnkleHipEnv):
    ...
    def _get_obs(self):
        ...
        return np.concatenate([
            q, qd,
            [com_y / self.eps_pos],  # normalized by eps_pos for reward shaping
            [self.target_y / self.eps_pos],  # normalized by eps_pos for reward shaping
            [(com_y - self.target_y) / self.eps_pos],
        ]).astype(np.float64)
    ...
    def reset(self, **kwargs):
        ...
        com_y = self._com_y()
        self.prev_norm_dist = abs(com_y - self.target_y) / self.eps_pos
    ...
    def step(self, action):
        ...
        norm_dist_raw = abs(com_y - self.target_y) / self.eps_pos
...
if __name__ == "__main__":
    ...
    model.save("ppo_candidate1_B")
    env.save("vecnormalize_candidate1_B.pkl")
    env.close()
