...
STAY PENALTY REMOVED. 

BASE OF SUPPORT (from XML, box half-extents "0.15 0.08 0.05"):
    x (AP) half-length = 0.15 m
    y (ML) half-width  = 0.08 m
Each axis is bounded against its own extent.

Usage:
    python candidate2_yaxis.py
"""
import os
...
EVERSION_J = 0   # primary ML driver
...
# Measured: +15 deg ankle_eversion -> com_y = -0.31058 m.
...
# Reduced from 0.25 / 1.0 -- see header note.
SAFETY_WEIGHT = 0.15
OFF_AXIS_WEIGHT = 0.30


class Candidate2Env(AnkleHipEnv):
    """Tracking on com_y (ML, task); safety on com_x (AP, perturbed)."""
    ...
    def __init__(self, fixed_target=None, omega=OMEGA,
                 ...
                 safety_weight=SAFETY_WEIGHT, off_axis_weight=OFF_AXIS_WEIGHT,
    ...
    def reset(self, **kwargs):
        ...
        # Solve directly for the eversion angle, then correct.
    ...
    def step(self, action):
        # Perturbation on axis 0 (= com_x = AP), orthogonal to the task.
...
if __name__ == "__main__":
    log_dir = "./training_logs_candidate2_yaxis_015_107/"
    ...
    model.save("ppo_candidate2_yaxis_015_107")
    env.save("vecnormalize_candidate2_yaxis_015_107.pkl")
    env.close()