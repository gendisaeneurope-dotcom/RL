"""
Candidate 3 (stay-penalty variant, matching Candidate 1/2's final fix):
energy + tracking (from Candidate F) + capture-point safety term
+ com_y stabilization + active stay-near-start penalty during the
pre-commit window.

Same base as Candidate 2 (XCoM) -- only the safety term differs
(capture-point excess penalty instead of XCoM velocity penalty).

Porting the exact fix pattern already proven on Candidates 1 and 2:
- Branch order: delay-window check happens BEFORE the success check, so
  no SUCCESS_BONUS is reachable while _current_step < TRACKING_DELAY_STEPS,
  regardless of position/velocity.
- SUCCESS_BONUS raised to 6.0 (from 2.0) -- confirmed on Candidate 2 to
  meaningfully increase post-delay movement magnitude.
- STAY_PENALTY_WEIGHT=0.5, TRACKING_DELAY_STEPS=723 -- same values as
  Candidates 1 and 2, for a fair three-way comparison (varying only the
  safety-term formulation, not the timing mechanism).
...
class Candidate3Env(AnkleHipEnv):
    """Candidate F's baseline (energy + tracking) plus capture-point safety
    + com_y stabilization + active stay-near-start penalty (branch-order fixed,
    matching Candidates 1 and 2's final version)."""
    ...
    def step(self, action):
        ...
        # tracking: FIXED BRANCH ORDER (matching Candidates 1 and 2). The
        # delay-window check comes BEFORE the success check, so no success
        # bonus is reachable while _current_step < TRACKING_DELAY_STEPS.
        stay_penalty = 0.0
        if failed:
