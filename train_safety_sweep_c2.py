"""
train_safety_sweep_c2.py  --  CANDIDATE 2 (XCoM velocity safety)
=============================================================
Axis-swapped: task on com_y (ML), safety on com_x (AP).


Usage:
    python train_safety_sweep_c2.py
"""
import os
...
class Candidate2EnvY(AnkleHipEnv):
    """Axis-swapped Candidate 2, parameterised by safety_weight."""

    def __init__(self, safety_weight=0.15, off_axis_weight=None,
    ...
    def step(self, action):
        ...
        # XCoM VELOCITY safety on the AP axis.
...
def train_one(safety_weight, seed):
    tag = f"c2_yaxis_sw{safety_weight:g}_s{seed}".replace(".", "")
    log_dir = f"./training_logs_{tag}/"
    os.makedirs(log_dir, exist_ok=True)

    def make_env(rank):
        def _f():
            e = Candidate2EnvY(safety_weight=safety_weight,
...
if __name__ == "__main__":
    ...
    if RUN_MODE == "seeds":
        print(f"C2 SEED REPLICATION: safety={SEED_CHECK_WEIGHT}, seeds={SEEDS}")
        for sd in SEEDS:
            print(f"\n=== C2 safety={SEED_CHECK_WEIGHT}, seed={sd} ===")
        ...
        print(f"C2 WEIGHT SWEEP: values={SAFETY_VALUES}, seed={SWEEP_SEED}")
        for sw in SAFETY_VALUES:
            print(f"\n=== C2 safety={sw} (off_axis={sw*2:g}), seed={SWEEP_SEED} ===")
    ...
    print("\n" + "=" * 84)
    print(f"CANDIDATE 2 (XCoM velocity safety) -- mode: {RUN_MODE}")
    ...
    if RUN_MODE == "seeds":
        ...
        print("(hit 10/20, 20/20, 10/20) -- i.e. variance dominated. If C2")
        print("shows the same spread, no weight can be claimed as selected.")