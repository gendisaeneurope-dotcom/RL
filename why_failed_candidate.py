...
Usage:
    python why_failed_candidate.py candidate1_A/B/C/D/D
...
CONFIGS = {
    "candidate1_A": dict(module="candidate1_A", cls="Candidate1Env",
                            ...
                            log_dir="./training_logs_candidate1/"),
...
def get_target_range(probe_env):
    module = sys.modules[type(probe_env).__module__]
    if hasattr(module, "TARGET_RANGE"):
        return float(module.TARGET_RANGE)
    if hasattr(probe_env, "target_range"):
        return float(probe_env.target_range)
    raise AttributeError("Could not find TARGET_RANGE on module or instance.")
...
def run(key):
    ...
    target_range = get_target_range(probe)
