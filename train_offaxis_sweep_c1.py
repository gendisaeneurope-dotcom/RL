"""
train_offaxis_sweep_c1.py
=========================
Candidate 1 (axis-swapped, NO safety term): off-axis weight sweep AND
seed replication.


TWO MODES -- set RUN_MODE below:
  "sweep"  -- one run per off-axis weight (screening)
  "seeds"  -- three seeds at a single weight (variance check)


C2 reference (XCoM velocity safety, 1M steps):
    safety 0.15 / off_axis 0.30 -> err 0.0004, 20/20 hit

Usage:
    python train_offaxis_sweep_c1.py
"""
import os
...
OFFAXIS_VALUES = [0.0, 0.15, 0.30, 0.60]   # used in "sweep" mode
SWEEP_SEED = 0
# ---------------------------------------------------------------------


class Candidate1EnvY(AnkleHipEnv):
    """Axis-swapped Candidate 1: no safety term, parameterised off_axis."""

    def __init__(self, off_axis_weight=0.30,
...
def train_one(off_axis_weight, seed):
    """Tag includes the seed so runs never overwrite one another."""
    tag = f"c1_yaxis_oa{off_axis_weight:g}_s{seed}".replace(".", "")
    log_dir = f"./training_logs_{tag}/"
    os.makedirs(log_dir, exist_ok=True)

    def make_env(rank):
        def _f():
            e = Candidate1EnvY(off_axis_weight=off_axis_weight,
    ...
    N_ENVS = 8
    env = SubprocVecEnv([make_env(i) for i in range(N_ENVS)], start_method=None)
    env.seed(seed)
...
def evaluate(tag, off_axis_weight, n_eps=20):
    model = PPO.load(f"ppo_{tag}")
    errors, lengths, ap_ptp, wrong_side = [], [], [], 0
    for ep in range(n_eps):
        venv = DummyVecEnv([lambda: TimeLimit(
            Candidate1EnvY(off_axis_weight=off_axis_weight), max_episode_steps=1000)])
...
if __name__ == "__main__":
    ...
    if RUN_MODE == "seeds":
        print(f"SEED REPLICATION: off_axis={SEED_CHECK_WEIGHT}, seeds={SEEDS}")
        for sd in SEEDS:
            print(f"\n=== C1 off_axis={SEED_CHECK_WEIGHT}, seed={sd} ===")
        ...
        print(f"WEIGHT SWEEP: values={OFFAXIS_VALUES}, seed={SWEEP_SEED}")
        for oa in OFFAXIS_VALUES:
            print(f"\n=== C1 off_axis={oa}, seed={SWEEP_SEED} ===")
            tag = train_one(oa, seed=SWEEP_SEED)
            r = evaluate(tag, oa)
            results.append((f"oa {oa:g}", oa) + r)
            print(f"  err {r[0]:.4f} | len {r[1]:.0f} | AP p-p {r[2]:.4f} "
                  f"| wrong {r[3]}/20 | hit {r[4]}/20")

    print("\n" + "=" * 80)
    print(f"CANDIDATE 1 (no safety term) -- mode: {RUN_MODE}")
    print(f"{'run':>10} {'off_axis':>9} {'mean_err':>10} {'len':>6} "
          f"{'AP p-p':>9} {'wrong':>8} {'hit':>7}")
    print("-" * 80)
    for label, oa, err, ln, ap, wrong, hit in results:
        print(f"{label:>10} {oa:>9.2f} {err:>10.4f} {ln:>6.0f} "
              f"{ap:>9.4f} {wrong:>6}/20 {hit:>5}/20")
    print("=" * 80)
    print("\nHuman AP peak-to-peak reference: 0.0077 m")
    print("C2 (XCoM safety 0.15 / off_axis 0.30): err 0.0004, 20/20 hit")
    if RUN_MODE == "seeds":
        print("\nIf these three differ substantially, the weight sweep was")
        print("measuring training variance rather than the weight itself.")